...
from ultralytics.engine.model import Model # type: ignore
...
def train_v2(self: YOLO, pruning=False, **kwargs):
    ...
    self.task = overrides.get('task', 'segment') or self.task
    self.trainer = self.task_map[self.task]['trainer'](overrides=overrides, _callbacks=self.callbacks) # change 
...
def prune(args):
    ...
    pruning_cfg['data'] = "tooth_dataset/tooth_data.yaml" # change 
    pruning_cfg['epochs'] = 3 # change 
    ...
    metric = validation_model.val(**pruning_cfg) # change 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', default='yolov8n-ghost.pt', help='Pretrained pruning target model file')
